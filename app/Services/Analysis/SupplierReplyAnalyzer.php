...
- "Уточняем цену / информацию / сроки / наличие", "уточняем / проверяем / разбираемся / выясняем (поставщик уточняет У СЕБЯ)", "Готовим КП", "Запрос принят/получен" → auto_reply, НЕ question (поставщик информирует, что работает, ответа от нас НЕ ждёт). Не извлекай такие фразы как question даже в составе mixed-письма.
...
- ОТКАЗ ОТ КАНАЛА: «заказы оформляются только через интернет-магазин / сайт / менеджеров», «заявки по email / телефону / форме не принимаются и не обрабатываются» — поставщик не работает по нашему запросу → email_type="rejection" (rejection_reason: not_our_profile), НЕ question.
- 🚫 ОТСУТСТВИЕ ЦЕНЫ — НЕ QUESTION: если поставщик не дал цену, НЕ создавай question вида «цены не указаны, уточните стоимость и сроки» — автор заявки цену поставщика назначить НЕ может. Нет цены → offer (если цена всё же есть / по товарной ссылке), rejection (если поставщик уклонился), empty_reply (пусто) или auto_reply («готовим КП / уточняем цену»). Question создавай ТОЛЬКО когда ПОСТАВЩИК сам задаёт вопрос НАМ.
- 🚨 ТОВАРНАЯ ССЫЛКА ≠ ОТПИСКА (приоритет над правилами отписок): если поставщик пишет «в наличии / цена на сайте / цена соответствует сайту» и даёт ссылку(и) на КОНКРЕТНЫЙ товар (/product/, /catalog/, /item/) — это НЕ rejection и НЕ question. Верни эти URL в fetch_urls (система загрузит страницы и извлечёт цену → offer). Rejection ставь ТОЛЬКО когда ссылка ведёт на ГЛАВНУЮ / контакты / дистрибьюторов, либо «email-заявки не принимаем», либо товара явно нет.
