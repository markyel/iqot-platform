...
TXT;
    ...
    private function normalizeBody(array $parsed): array
    {
        ...
        if ($closing === '' || mb_strlen($closing) < 10) {
            $closing = 'Укажите, пожалуйста, цену, сроки поставки и условия оплаты.';
        }
    ...
    private function stripPolitenessTail(string $closing): string
    {
        ...
        $politeness = '/(с\s+нетерпением|жд[ёеу]м?\b|ожида[ею]\w*|наде[ёе]мся|надеюсь|рассчитыва[ею]\w*|заранее\s+благодар\w+|благодар\w+\s+за|призна\w+\s+за|спасибо)/iu';
