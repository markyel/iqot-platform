...
class PositionAffinity
{
    ...
    private function avoidPairs(array $group): array
    {
        ...
        $sys = 'Ты помогаешь группировать позиции (комплектующие для лифтов/эскалаторов) в запросы поставщикам. '
            . 'Один запрос поставщику со СПИСКОМ разнотипных позиций одной закупки — это НОРМА B2B '
            . '(кнопки, реле, платы, контакторы и т.п. вместе — нормально). Вернуть строго JSON.';
        $user = "Позиции (индекс: описание):\n" . implode("\n", $lines)
            . "\n\nНайди только пары, которые объединять НЕЛЬЗЯ — настолько разные по отрасли/назначению, "
            . "что один поставщик почти наверняка не торгует обоими (например, механическая ступень "
            . "эскалатора и электронная плата управления). Обычные разнотипные комплектующие одной "
            . "закупки НЕ считаются такой парой — не перечисляй пары «на всякий случай». "
            . "Верни JSON: {\"avoid\": [[i,j], ...]} — индексы; если таких пар нет — {\"avoid\": []}.";
