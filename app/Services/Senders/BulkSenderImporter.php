...
2. sender_full_name — полное ФИО (Фамилия Имя Отчество).
   ВАЖНО: если локальная часть «{$emailLocal}» похожа на имя, фамилию или их часть
   (например «segio» → Сергей, «baev» → Баев, «evgen» → Евгений, «a.petrov» → Петров,
   «i.ivanova» → Иванова), то ФИО ДОЛЖНО перекликаться с ней: бери намёк из локальной
   части и достраивай реалистичные имя/отчество (учитывай род — женское или мужское).
   Только если в локальной части нет ничего похожего на имя (например «info», «sales»,
   «zakupki», «office123»), придумай ФИО сам. Имена бери ЖИВЫЕ и РАЗНООБРАЗНЫЕ —
   не используй шаблонные «Иванов Иван Иванович» / «Сидоров Алексей Иванович».
