...
- **`IdentificationAnalyzer`** — AI-сопоставление (порт «Prepare AI Prompt» + «AI Agent» + активного «Parse AI Response»; усиленный «Parse AI Response1» в графе n8n — орфан, не подключён). Системный промпт — дословный порт: классификация письма (автоответ/вопрос/КП/отказ), сопоставление ПО НАЗВАНИЮ товара (артикулы поставщика ≠ наши), учёт токен-метки ⭐. Зовёт `OpenAIClassifierClient::jsonCompletion` (`EMAILS_IDENTIFY_MODEL`, json_object, temperature 0). Валидация: `queue_id` обязан быть среди кандидатов И `confidence ≥ min_confidence` → `validation_passed=true`; токен-матч поднимает confidence до 0.9. Фолбэк при ошибке AI — `validation_passed=false` (письмо уйдёт в manual_review).
- **`IdentifiedEmailPersister`** — запись результата (порт «Update as Identified / Find+Create Conversation / Save Email Message / Migrate Attachments» + ветки manual_review). Успех: транзакцией `unidentified_emails.status='identified'` + `identified_*` + метод/уверенность; беседа (найти по `(batch_id, supplier_id)` или создать `status='waiting'`, `items_total` из `email_batches.items_count`); `email_messages` (`direction='incoming'`, дедуп по `message_id`); при создании нового сообщения — миграция вложений в `email_attachments` (с `local_path`) + `email_queue.status='replied'`. Неуспех: `status='manual_review'` (+`reason='no_match'` для ветки без кандидатов).
