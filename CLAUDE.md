...
- **`WebPageFetcher`** — замена Tavily 2-шаговым сёрфингом: если AI вернул `fetch_urls` и в офферах нет цены → грузим до `fetch_max` страниц (`Http`, только http/https, `strip_tags`, обрезка `fetch_chars`), дописываем их текст в userPrompt и делаем второй прогон AI. Не tool-calling.
  - **⚠ ОГРАНИЧЕНИЕ (отложено до апгрейда RAM).** Часто цена поставщика НЕ в письме, а на его сайте по ссылке; страница за JS/cookie-анти-ботом (напр. Beget отдаёт 274-байт заглушку, ставящую cookie `beget=begetok` и `location.reload()`). Простой `Http::get` получает только заглушку → после `strip_tags` пусто → цена не извлекается (оффер пишется с `price=0`, в `saveOffer` пропускается). Tavily в n8n это обходил, исполняя JS. **Решение (выбор пользователя): headless-браузер (Spatie Browsershot + Chromium/Puppeteer).** Заблокировано: прод 2 ГБ RAM, занято ~1.5 ГБ, swap=0 — Chrome словит OOM. **Сначала апгрейд RAM (хостер, на стороне пользователя), затем реализация headless-fetch.** Node 20 на проде есть, Chrome — нет.
